...
def train(erase_concept, erase_from, train_method, iterations, negative_guidance, lr, save_path, config, ckpt_path):
    nsteps = 50

    diffuser = StableDiffuser(scheduler='DDIM', config=config, ckpt_path=ckpt_path).to('cuda')
    diffuser.train()
    ...
    for i in pbar:
        with torch.no_grad():
            ...
            positive_latents = diffuser.predict_noise(iteration, latents_steps[0], positive_text_embeddings,
                                                      guidance_scale=1)
            neutral_latents = diffuser.predict_noise(iteration, latents_steps[0], neutral_text_embeddings,
                                                     guidance_scale=1)
            target_latents = diffuser.predict_noise(iteration, latents_steps[0], target_text_embeddings,
                                                    guidance_scale=1)
            if erase_concept_sampled[0] == erase_concept_sampled[1]:
                target_latents = neutral_latents.clone().detach()
        with finetuner:
            negative_latents = diffuser.predict_noise(iteration, latents_steps[0], target_text_embeddings,
                                                      guidance_scale=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_method', help='Type of method (xattn, noxattn, full, xattn-strict', type=str,
                        required=True)
    ...
    train(erase_concept=erase_concept, erase_from=erase_from, train_method=train_method, iterations=iterations,
          negative_guidance=negative_guidance, lr=lr, save_path=save_path,
