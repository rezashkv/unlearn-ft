...
from ..utils.estimation_utils import gumbel_softmax_sample, hard_concrete, importance_gumbel_softmax_sample
...
class StructureVectorQuantizer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        template = torch.repeat_interleave(template, template).type(dtype)
    ...
    def gumbel_sigmoid_trick(self, z_q: torch.Tensor):
        ...
        z_q_depth_b_ = importance_gumbel_softmax_sample(z_q_depth, temperature=self.temperature, offset=self.base,
                                                        fixed_seed=fixed_seed).to(dtype=self.dtype)
        z_q_depth_b = torch.zeros_like(z_q_depth_b_, device=z_q_depth_b_.device, dtype=self.dtype)
        if self.order_depth:
        ...
        z_q_width_b_list = [gumbel_softmax_sample(zw, temperature=self.temperature, offset=self.base,
                                                  force_width_non_zero=self.non_zero_width, fixed_seed=fixed_seed).to(dtype=self.dtype) for
