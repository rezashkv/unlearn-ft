...
from diffusers.models.normalization import AdaGroupNorm, AdaLayerNormZeroSingle, AdaLayerNormZero, RMSNorm, \
...
from ..models.attention import GatedAttention as GatedFluxAttention
...
def gated_flux_attention_counter_hook(gated_attention_module, input, output):
    ...
    if gated_attention_module.total_macs == 0. or gated_attention_module.pruned:
        ...
        # norm_q
        if gated_attention_module.norm_q is not None:
            gated_attention_module.total_macs += gated_attention_module.norm_q.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.norm_q.__macs__

        # norm_k
        if gated_attention_module.norm_k is not None:
            gated_attention_module.total_macs += gated_attention_module.norm_k.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.norm_k.__macs__

        if hasattr(gated_attention_module, 'add_q_proj'):
            # add_q_proj
            gated_attention_module.total_macs += gated_attention_module.add_q_proj.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.add_q_proj.__macs__

            # add_k_proj
            gated_attention_module.total_macs += gated_attention_module.add_k_proj.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.add_k_proj.__macs__

            # add_v_proj
            gated_attention_module.total_macs += gated_attention_module.add_v_proj.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.add_v_proj.__macs__

            # norm_added_q
            if gated_attention_module.norm_added_q is not None:
                gated_attention_module.total_macs += gated_attention_module.norm_added_q.__macs__
                gated_attention_module.prunable_macs += gated_attention_module.norm_added_q.__macs__

            # norm_added_k
            if gated_attention_module.norm_added_k is not None:
                gated_attention_module.total_macs += gated_attention_module.norm_added_k.__macs__
                gated_attention_module.prunable_macs += gated_attention_module.norm_added_k.__macs__


        attn_macs = 0
        batch_size, seq_len = output[0].shape[0], output[0].shape[1]
        if hasattr(gated_attention_module, 'add_q_proj'): # GatedFluxTransformerBlock
            seq_len += output[1].shape[1]
        dim = gated_attention_module.to_q.out_features
        num_heads, head_dim = gated_attention_module.heads, dim // gated_attention_module.heads
        ...
        if hasattr(gated_attention_module, 'to_add_out'): # GatedFluxTransformerBlock
            # to_out
            gated_attention_module.total_macs += gated_attention_module.to_out[0].__macs__
            gated_attention_module.prunable_macs += gated_attention_module.to_out[0].__macs__

            # to_add_out
            gated_attention_module.total_macs += gated_attention_module.to_add_out.__macs__
            gated_attention_module.prunable_macs += gated_attention_module.to_add_out.__macs__
...
def joint_attention_counter_hook(attention_module, input, output):
    attention_module.total_macs, attention_module.prunable_macs = 0., 0.

    # to_q
    attention_module.total_macs += attention_module.to_q.__macs__
    attention_module.prunable_macs += attention_module.to_q.__macs__

    # to_k
    attention_module.total_macs += attention_module.to_k.__macs__
    attention_module.prunable_macs += attention_module.to_k.__macs__

    # to_v
    attention_module.total_macs += attention_module.to_v.__macs__
    attention_module.prunable_macs += attention_module.to_v.__macs__

    # norm_q
    if attention_module.norm_q is not None:
        attention_module.total_macs += attention_module.norm_q.__macs__
        attention_module.prunable_macs += attention_module.norm_q.__macs__

    # norm_k
    if attention_module.norm_k is not None:
        attention_module.total_macs += attention_module.norm_k.__macs__
        attention_module.prunable_macs += attention_module.norm_k.__macs__

    if hasattr(attention_module, 'add_q_proj'):
        # add_q_proj
        attention_module.total_macs += attention_module.add_q_proj.__macs__
        attention_module.prunable_macs += attention_module.add_q_proj.__macs__

        # add_k_proj
        attention_module.total_macs += attention_module.add_k_proj.__macs__
        attention_module.prunable_macs += attention_module.add_k_proj.__macs__

        # add_v_proj
        attention_module.total_macs += attention_module.add_v_proj.__macs__
        attention_module.prunable_macs += attention_module.add_v_proj.__macs__

        # norm_added_q
        if attention_module.norm_added_q is not None:
            attention_module.total_macs += attention_module.norm_added_q.__macs__
            attention_module.prunable_macs += attention_module.norm_added_q.__macs__

        # norm_added_k
        if attention_module.norm_added_k is not None:
            attention_module.total_macs += attention_module.norm_added_k.__macs__
            attention_module.prunable_macs += attention_module.norm_added_k.__macs__
    ...
    attention_module.total_macs += attn_macs
    attention_module.prunable_macs += attn_macs

    # to_out
    attention_module.total_macs += attention_module.to_out[0].__macs__
    attention_module.prunable_macs += attention_module.to_out[0].__macs__

    if not attention_module.context_pre_only:
        if hasattr(attention_module, 'to_add_out'):
            # to_add_out
            attention_module.total_macs += attention_module.to_add_out.__macs__
            attention_module.prunable_macs += attention_module.to_add_out.__macs__

    attention_module.__macs__ = attention_module.total_macs
...
MODULES_MAPPING = {
    ...
    AdaLayerNormZeroSingle: ada_layer_norm_zero_macs_counter_hook,
    AdaLayerNormZero: ada_layer_norm_zero_macs_counter_hook,
    RMSNorm: rms_norm_macs_counter_hook,
    AdaLayerNormContinuous: ada_layer_norm_zero_macs_counter_hook,
    ...
    Attention: gated_attention_counter_hook,
    GatedAttention: gated_attention_counter_hook,
    GatedFluxAttention: gated_flux_attention_counter_hook,
