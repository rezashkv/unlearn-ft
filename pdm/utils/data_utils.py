...
def get_dataset(config):
    ...
    if "conceptual_captions" in data_dir:
        ...
        if "parti-prompts" in config.dataset_name:
            # add an index column to the dataset. 1 to size of dataset
            # Adding an index column
            indices = list(range(0, len(dataset["train"])))  # 1 to size of dataset
            dataset["train"] = dataset["train"].add_column("index", indices)
            dataset["validation"] = dataset["train"]
        if "validation" not in dataset:
            dataset_ = dataset["train"].train_test_split(test_size=0.083333, seed=42)
...
def preprocess_sample(samples, tokenizers, text_encoders, mpnet_model, mpnet_tokenizer, transform, image_column="image",
    ...
    if len(tokenizers) > 1:
        ...
        samples["empty_prompt_embeds"] = encode_prompt(tokenizers[0], text_encoders[0],
                                                       ["" for _ in samples[caption_column]] if len(
                                                           samples[caption_column]) > 0 else "",
                                                       max_sequence_length[0], device=device)
    else:
        samples["prompt_embeds"] = encode_prompt(tokenizers[0], text_encoders[0], samples[caption_column],
                                                 max_sequence_length, device=device)
        samples["empty_prompt_embeds"] = encode_prompt(tokenizers[0], text_encoders[0],
                                                       ["" for _ in samples["caption"]] if len(samples["caption"]) > 0 else "",
                                                       max_sequence_length, device=device)
...
def collate_fn(samples):
    ...
    empty_prompt_embeds = torch.stack([sample["empty_prompt_embeds"] for sample in samples])
    empty_prompt_embeds = empty_prompt_embeds.to(memory_format=torch.contiguous_format).float()
    ...
    return_dict = {"pixel_values": pixel_values, "prompt_embeds": prompt_embeds,
...
def filter_dataset(dataset, hyper_net, quantizer, mpnet_model, mpnet_tokenizer, caption_column="caption"):
    ...
    with torch.no_grad():
        for batch in train_filtering_dataloader:
            batch = encode_with_mpnet(batch, mpnet_model, mpnet_tokenizer)
