...
from diffusers import UNet2DConditionModel, AutoencoderKL, FluxPipeline
...
from diffusers.utils import replace_example_docstring, is_transformers_available, is_torch_xla_available
...
from ..models import HyperStructure, StructureVectorQuantizer
...
class StableDiffusionPruningPipeline(StableDiffusionPipeline):
    ...
    def sample_progressive(
        ...
        if not output_type == "latent":
            ...
            image_nsfw = [self.run_safety_checker(image, device, prompt_embeds.dtype) for image in images]
            images, has_nsfw_concept = zip(*image_nsfw)
        else:
            images = latents
            has_nsfw_concept = None
        ...
        if has_nsfw_concept[0] is None:
            do_denormalize = [True] * images.shape[0]
        else:
            do_denormalize = [[not has_nsfw for has_nsfw in has_nsfw_conc] for has_nsfw_conc in has_nsfw_concept]
...
class FluxPruningPipeline(FluxPipeline):
    _optional_components = ["hyper_net", "quantizer"]
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        hyper_net = kwargs.pop("hyper_net", None)
        quantizer = kwargs.pop("quantizer", None)
        pipe.hyper_net = hyper_net
        pipe.quantizer = quantizer
        return pipe
    ...
    def __call__(
        ...
        self.transformer.set_structure(arch_vectors_separated)
        ...
        macs_dict = self.transformer.calc_macs()
        resource_ratios = macs_dict['cur_prunable_macs'] / (
            self.transformer.resource_info_dict['cur_prunable_macs'].squeeze())
        ...
        if not return_dict:
            if return_mapped_indices:
                return (image, min_encoding_indices)
            return (image,)
        else:
            if return_mapped_indices:
                return FluxPipelineOutput(images=image), min_encoding_indices, resource_ratios
